...
class LogArchiveService:
    ...
    def _get_all_status_from_redis(task_id: str) -> List[Dict[str, Any]]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                stream_key = realtime_log_service.status_stream_key

                # 获取所有消息并过滤 execution_id
                messages = realtime_log_service.redis_client.xrange(stream_key)
    ...
    def _cleanup_redis_streams(task_id: str):
        ...
        try:
            ...
            status_stream_key = realtime_log_service.status_stream_key
    ...
    @staticmethod
    def _aggregate_logs_from_entries(logs: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        将 Redis Stream 日志条目聚合为 step_logs 结构（steps -> host_logs），便于序列化器消费。
        """
        step_logs: Dict[str, Any] = {}

        for entry in logs:
            step_name = entry.get('step_name') or '执行任务'
            step_order = entry.get('step_order') or 1
            host_id = str(entry.get('host_id') or 'unknown')
            host_name = entry.get('host_name') or host_id
            host_ip = entry.get('host_ip') or ''
            log_type = entry.get('log_type') or 'stdout'
            ...
            host_logs = step_logs[step_id]['host_logs']
            if host_id not in host_logs:
                host_logs[host_id] = {
                    'host_id': host_id,
                    'host_name': host_name,
            ...
            host_entry = host_logs[host_id]
            if timestamp:
                if not host_entry['start_time'] or timestamp < host_entry['start_time']:
                    host_entry['start_time'] = timestamp
                if not host_entry['end_time'] or timestamp > host_entry['end_time']:
                    host_entry['end_time'] = timestamp

            formatted = f"[{timestamp}] {content}\n" if content else ''
            if log_type in ['stderr', 'error']:
                host_entry['error_logs'] += formatted
                host_entry['status'] = 'failed'
            else:
                host_entry['logs'] += formatted
                if host_entry['status'] != 'failed':
                    host_entry['status'] = 'success'
            if content:
                host_entry['log_count'] += 1

        # 反推步骤状态
        for step_data in step_logs.values():
            hosts = step_data.get('host_logs', {}).values()
            if any(h.get('status') == 'failed' for h in hosts):
                step_data['status'] = 'failed'
            elif all(h.get('status') == 'success' for h in hosts if hosts):
    ...
    @staticmethod
    def _build_summary_from_step_logs(step_logs: Dict[str, Any]) -> Dict[str, int]:
        """根据 step_logs 粗略计算摘要信息"""
        total_steps = len(step_logs)
        total_hosts = 0
        success_hosts = 0
        failed_hosts = 0

        for step in step_logs.values():
            hosts = step.get('host_logs', {})
            total_hosts += len(hosts)
        ...
        return {
            'total_steps': total_steps,
            'success_steps': sum(1 for s in step_logs.values() if s.get('status') == 'success'),
            'failed_steps': sum(1 for s in step_logs.values() if s.get('status') == 'failed'),
    ...
    @staticmethod
    def get_execution_logs_by_pointer(log_pointer: str, limit: int = 500) -> Dict[str, Any]:
        """
        通过 storage_uri 指针（redis:job_logs/<task_id>@<id>）获取最新日志并聚合为 step_logs。
        """
        try:
            if not log_pointer:
                return {'success': False, 'message': 'log_pointer 为空'}

            logs = realtime_log_service.get_logs_by_pointer(log_pointer, limit=limit) or []
            if not logs:
                return {'success': False, 'message': '未找到日志'}

            step_logs = LogArchiveService._aggregate_logs_from_entries(logs)
            summary = LogArchiveService._build_summary_from_step_logs(step_logs)
            ...
            logger.error(f"通过指针获取执行日志失败 pointer={log_pointer}: {e}")
