...
@method_decorator(csrf_exempt, name='dispatch')
class AgentUninstallProgressSSEView(SSEBaseView):
    """Agent 卸载进度 SSE 视图"""
    ...
    def get_resource_id(self, kwargs):
        """获取卸载任务 ID"""
        return kwargs.get('uninstall_task_id')

    def check_permission(self, user, uninstall_task_id):
        """检查用户是否有权限访问卸载任务"""
        try:
            from apps.agents.models import AgentUninstallRecord

            uninstall_record = AgentUninstallRecord.objects.filter(
                uninstall_task_id=uninstall_task_id
            ).first()

            if not uninstall_record:
                logger.warning(f"卸载任务不存在: {uninstall_task_id}")
                return False
            ...
            # 卸载者权限
            if uninstall_record.uninstalled_by == user:
                return True
            ...
            return uninstall_record.host in allowed_hosts

        except Exception as e:
            logger.error(f"权限检查异常: {uninstall_task_id} - {e}")
            return False

    def get(self, request, uninstall_task_id):
        """获取卸载进度流"""
        logger.info(f"SSE卸载进度连接: 用户={request.user.username}, 任务ID={uninstall_task_id}")
        ...
        async def event_stream():
            status_prefix = getattr(settings, "INSTALL_STATUS_STREAM_PREFIX", "agent_install_status:")
            log_stream_key = getattr(settings, "INSTALL_LOG_STREAM_KEY", "agent_install_logs")
            status_stream_key = f"{status_prefix}{uninstall_task_id}"
            status_last_id = last_id
            log_last_id = last_id
            finished = False
            ...
            try:
                ...
                yield self.format_sse_message({
                    'type': 'connection_established',
                    'message': f'已连接到卸载任务 {uninstall_task_id} 的进度流',
                    'uninstall_task_id': uninstall_task_id
                }).encode('utf-8')
                ...
                if last_id == '0':
                    try:
                        ...
                        # 获取历史日志
                        logger.info(f"获取历史日志: uninstall_task_id={uninstall_task_id}, stream_key={log_stream_key}")
                        historical_logs = await self.get_historical_logs_async(
                            uninstall_task_id, limit=50, stream_key=log_stream_key
                        )
                        logger.info(f"发送历史日志: {len(historical_logs)} 条")
                        for log in historical_logs:
                            normalized = self.normalize_log_message(log, uninstall_task_id)
                            log_last_id = normalized.get('id') or log.get('id', '0')
                            yield self.format_sse_message({
                                'type': 'log', **normalized
                            }, event_id=log_last_id).encode('utf-8')
                ...
                # 如果历史中已经包含终态，则直接结束
                if finished:
                    logger.info(f"卸载任务 {uninstall_task_id} 已达终态（历史），关闭 SSE")
                    return

                # 开始实时合并流
                logger.info(f"开始实时进度流: uninstall_task_id={uninstall_task_id}")
                consecutive_errors = 0

                while True:
                    try:
                        if finished:
                            logger.info(f"卸载任务 {uninstall_task_id} 已达终态，关闭实时进度流")
                            break
                            ...
                            for stream, msgs in messages:
                                for msg_id, fields in msgs:
                                    if stream == status_stream_key:
                                        ...
                                        exec_id = fields.get('execution_id') or fields.get('task_id')
                                        if str(exec_id) != str(uninstall_task_id):
                                            continue
                                        log_last_id = msg_id
                                        normalized = self.normalize_log_message(fields, uninstall_task_id)
                                        yield self.format_sse_message({
                                            'type': 'log', **normalized
                                        }, event_id=msg_id).encode('utf-8')
                        else:
                        ...
                        consecutive_errors += 1
                        logger.error(f"实时进度流异常 ({consecutive_errors}/{self.max_consecutive_errors}): {uninstall_task_id} - {e}")
                        yield self.format_sse_message({
                            'type': 'error',
                            'message': f'实时流异常: {str(e)}'
                        }).encode('utf-8')
                        ...
                        if consecutive_errors >= self.max_consecutive_errors:
                            logger.error(f"实时进度流错误过多，退出: {uninstall_task_id}")
                            break

                        await asyncio.sleep(1)

            except Exception as e:
                logger.error(f"SSE进度流异常: {uninstall_task_id} - {e}")
                yield self.format_sse_message({
                    'type': 'error',
                    'message': str(e)
                }).encode('utf-8')
