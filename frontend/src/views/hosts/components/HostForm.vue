<template>
  ...
  >
    ...
    >
      ...
      <div class="form-tip">
        <icon-info-circle />
        <span>至少需要配置内网 IP 或外网 IP 之一</span>
      </div>
      <a-row :gutter="16">
        <a-col :span="12">
      ...
      <a-row :gutter="16">
        <a-col :span="12">
          <a-form-item label="标签 (键值对，可选)" field="tags">
            <div class="tags-editor">
              <div v-if="tagEntries.length === 0" class="tags-empty">
                <a-empty description="暂无标签" size="small" />
              </div>
              <div v-else class="tags-list">
                <div
                  v-for="(item, index) in tagEntries"
                  :key="index"
                  class="tag-row"
                >
                  <a-input
              ...
              <div class="tags-input-row">
                <a-button type="outline" size="small" @click="addTagRow">
...
const formRef = ref()
const tagEntries = reactive<Array<{ key: string; value: string }>>([])
...
tagEntries.splice(0, tagEntries.length)
}
  ...
  (host) => {
    if (host) {
      ...
      if (host.tags && Array.isArray(host.tags)) {
        host.tags.forEach((t: any) => {
          // 兼容对象形式 {key, value} 及字符串形式 "key=value"
