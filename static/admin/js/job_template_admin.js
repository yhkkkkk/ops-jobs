...
(function($) {
    ...
    function initStepTemplates() {
        ...
        if (!$('.step-template-selector').length) {
            ...
            var templates = [
                ...
                { name: '文件上传', type: 'file_transfer', transfer_type: 'upload' },
                { name: '文件下载', type: 'file_transfer', transfer_type: 'download' }
            ];
    ...
    function addStepFromTemplate(template) {
        ...
        setTimeout(function() {
            ...
            if (template.transfer_type) {
                newStep.find('select[name*="transfer_type"]').val(template.transfer_type);
            }
