...
func (a *Agent) performHealthCheck() *HealthStatus {
	...
	// 检查CPU使用率（如果超过90%，认为不健康）
	if systemInfo.CPUUsage > 90.0 {
		health.Healthy = false
		health.Issues = append(health.Issues, fmt.Sprintf("CPU usage too high: %.1f%%", systemInfo.CPUUsage))
	}

	// 检查内存使用率（如果超过95%，认为不健康）
	if systemInfo.MemoryUsage > 95.0 {
		health.Healthy = false
		health.Issues = append(health.Issues, fmt.Sprintf("Memory usage too high: %.1f%%", systemInfo.MemoryUsage))
	}

	// 检查磁盘空间（检查根目录或主要挂载点的使用率）
	rootDiskUsage := systemInfo.DiskUsage["/"] // Linux/macOS根目录
	if rootDiskUsage == 0 {
		rootDiskUsage = systemInfo.DiskUsage["C:"] // Windows C盘
	}
	if rootDiskUsage > 95.0 { // 如果使用率超过95%，认为不健康
		health.Healthy = false
		health.Issues = append(health.Issues, fmt.Sprintf("Disk usage too high: %.1f%%", rootDiskUsage))
	}
