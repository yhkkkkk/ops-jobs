...
@admin.register(DefaultScriptTemplate)
class DefaultScriptTemplateAdmin(GuardedModelAdmin):
    list_display = ['script_type', 'description', 'created_at', 'updated_at', 'permission_actions']
    list_filter = ['script_type', 'created_at']
    readonly_fields = ['created_at', 'updated_at']
    
    fieldsets = (
        ('基本信息', {
            'fields': ('script_type', 'description')
        }),
        ('模板内容', {
    ...
    def permission_actions(self, obj):
        """显示权限管理操作"""
        if obj:
            url = reverse('admin:script_templates_defaultscripttemplate_change', args=[obj.id])
            return format_html(
                '<a href="{}" class="button">管理权限</a>',
                url
            )
    ...
    def get_queryset(self, request):
        ...
        if request.user.is_superuser:
            return super().get_queryset(request)
        return get_objects_for_user(request.user, 'script_templates.change_defaultscripttemplate')
    ...
    def has_change_permission(self, request, obj=None):
        ...
        if request.user.is_superuser:
            return True
        if obj is None:
            return request.user.has_perm('script_templates.change_defaultscripttemplate')
        return request.user.has_perm('script_templates.change_defaultscripttemplate', obj)
    ...
    def has_delete_permission(self, request, obj=None):
        ...
        if request.user.is_superuser:
            return True
        if obj is None:
            return request.user.has_perm('script_templates.delete_defaultscripttemplate')
        return request.user.has_perm('script_templates.delete_defaultscripttemplate', obj)
