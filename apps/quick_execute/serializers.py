...
class QuickFileTransferSerializer(serializers.Serializer):
    ...
    # 统一使用 sources 描述来源；仅支持 local 上传或已入库的 server(download_url/storage_path)
    sources = serializers.ListField(
        child=serializers.DictField(),
        required=True,
        allow_empty=False,
        help_text="文件来源数组，示例: {type:'local',file_field:'local_file_0',remote_path:'/tmp/x'} 或 {type:'server',download_url:'https://...',remote_path:'/tmp/a.tar.gz'}"
    )
    # 执行账号相关
        ...
        for s in sources:
            ...
            if stype == 'local':
                if not s.get('file_field'):
                    raise serializers.ValidationError("local 类型的 source 必须包含 file_field（multipart 中文件字段名）")
                if not s.get('remote_path'):
                    raise serializers.ValidationError("local 类型的 source 必须包含 remote_path")
            else:
                # server 源：必须提供源服务器信息，控制面负责 HTTP 拉取到制品库
                if not s.get('source_server_host') or not s.get('source_server_path'):
                    raise serializers.ValidationError("server 类型的 source 必须包含 source_server_host 与 source_server_path")
                if s.get('account_id') is None or not isinstance(s.get('account_id'), int):
                    raise serializers.ValidationError("server 类型的 source 必须包含 account_id（整数）")
                # remote_path 可选，后端兜底
                # 可选的下载认证头
                if s.get('auth_headers') and not isinstance(s.get('auth_headers'), dict):
                    raise serializers.ValidationError("server 类型的 source 中 auth_headers 必须为对象")
